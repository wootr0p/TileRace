...
bool PollFieldInput(char* buf, int maxlen) {
    ...
    if ((IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL)) && IsKeyPressed(KEY_V)) {
        ...
        if (clip && clip[0]) {
            const int clip_len = (int)std::strlen(clip);
            const int copy_len = (clip_len < maxlen - 1) ? clip_len : maxlen - 1;
            std::memcpy(buf, clip, copy_len);
            buf[copy_len] = '\0';
            changed = true;
        }
        // Consume any chars generated by the V key press
