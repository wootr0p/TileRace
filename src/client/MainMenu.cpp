...
// ShowSplashScreen
// ---------------------------------------------------------------------------
void ShowSplashScreen(Font& font, int gamepad_index) {
    ...
    while (!WindowShouldClose()) {
        ...
        const bool gp = IsGamepadAvailable(gamepad_index);
        ...
        if (!skip_input) {
            ...
            if (gp) {
                for (int b = 0; b <= GAMEPAD_BUTTON_RIGHT_THUMB; b++)
                    if (IsGamepadButtonPressed(gamepad_index, (GamepadButton)b)) { any_input = true; break; }
            }
        }
